...
桌面版的数据默认保存在 `%LOCALAPPDATA%\SnowFlake\snowflake.db`，窗口状态、WebView 数据和日志也位于该目录。首次启动时会自动创建数据库；如果 EXE 旁边存在 `snowflake.db`，应用会将其复制到用户数据目录。升级或替换 EXE 不会覆盖正在使用的数据库。
...
```text
SnowFlake.exe [--db 数据库路径] [--portable] [--empty] [--debug]
```

- `--db`：使用指定数据库；
- `--portable`：将数据库保存在 EXE 旁边；
- `--empty`：首次启动时不迁移 EXE 旁边的旧数据库；
- `--debug`：启用桌面窗口调试模式。
...
脚本会创建独立的 `.venv-desktop` 环境、安装 `requirements-desktop.txt` 中的依赖、生成应用图标并通过 PyInstaller 构建目录版应用。默认发布包不包含数据库，以免误传个人作品。如需制作带有项目根目录种子数据库的内部版本，可执行 `./build-desktop.ps1 -IncludeSeedDatabase`。
...
默认仅监听 `http://localhost:10000`，数据库默认为程序目录下的 `snowflake.db`。
